...
function DHD{
    ...
    if($DHD -eq $true){
        ...
        try{
            [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
            Invoke-WebRequest -Uri https://github.com/labovaneng/common-wagens/blob/master/DHD_config_instellingen3.pdf -OutFile "DHD_config_instellingen3.pdf" -ErrorAction Stop
...
function ReplaceLaadSettings{
	...
	Set-Acl -Path "C:\Users\ENG\Desktop\Admin Tools\Scripts\LaadSettings.ps1" -AclObject $acl
        ...
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        Invoke-WebRequest -Uri https://raw.githubusercontent.com/labovaneng/settingsV12/master/LaadSettings.ps1 -OutFile LaadSettings.ps1
...
function DownloadDocumentatie{
    ...
    if($docs -eq $true){
        ...
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        Invoke-WebRequest -Uri https://github.com/labovaneng/common-wagens/blob/master/documentatie_wagens.zip?raw=true -OutFile documentatie.zip
...
function ReplaceBackupSettings{
    ...
    if($BackupSettings -eq $true){
        ...
        wget https://raw.githubusercontent.com/labovaneng/settingsV12/master/BackupSettings.ps1 -OutFile BackupSettings.ps1
...
function UpdateEncoder{
    ...
    Write-Host "encoder uptodate"
    	...
    	[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        Invoke-WebRequest -Uri https://github.com/labovaneng/coder/blob/master/Encoder_V35_16ch.exe?raw=true -OutFile Encoder_V35_16ch.exe
        ...
        $ruw = Test-Path "C:\Users\ENG\Desktop\Admin Tools\Scripts\Presets\Custom\RUW.sqpreset"
            ...
            if($montage -and $ruw -eq $true){
                ...
                try{
                    Set-Location $env:TEMP
	                [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
                    Invoke-WebRequest -Uri https://raw.githubusercontent.com/labovaneng/algemeenDocu/master/Montage.sqpreset -OutFile MONTAGE.sqpreset -ErrorAction Stop
                    Invoke-WebRequest -Uri https://raw.githubusercontent.com/labovaneng/algemeenDocu/master/RUW.sqpreset  -OutFile RUW.sqpreset -ErrorAction Stop
            ...
            if($loudness -and $zloudness -eq $true){
                ...
                Try{
                    Write-Host "Download Presets"
                     Set-Location $env:TEMP
	                [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
                    Invoke-WebRequest -Uri https://raw.githubusercontent.com/labovaneng/algemeenDocu/master/FASPEX%20met%20loudness.epr -OutFile "Faspex met loudness.epr" -ErrorAction Stop
                    Invoke-WebRequest -Uri https://raw.githubusercontent.com/labovaneng/algemeenDocu/master/Faspex%20zonder%20loudness.epr  -OutFile "Faspex zonder loudness.epr" -ErrorAction Stop
...
function ClearAdobeMediaCache{
	$limit = (Get-Date).AddDays(-10)
	$MediaCache = "E:\mediacachedatabase\Media Cache"
		# Delete files older than the $limit.
    ...
    $CacheFiles = "E:\mediacache\Media Cache Files"
        # Delete files older than the $limit.
    ...
    $PeakFiles = "E:\mediacache\Peak Files"
        # Delete files older than the $limit.
